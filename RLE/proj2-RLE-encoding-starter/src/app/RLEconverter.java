...
public class RLEconverter {
    ...
    *  of all compressed lines (the two characters are written in the first line)
...
public String compressLine(String line, char[] fileChars){
  ...
  public String[] compressAllLines(String[] lines, int dataSize, char[] fileChars){
      //TODO: Implement this method
      String[] compressedAllLine = new String[dataSize];
      for (int i=0; i < dataSize; i++){
        compressedAllLine[i] = compressLine(lines[i], fileChars); 
      }
    
      return compressedAllLine;
}
...
public String getCompressedFileStr(String[] compressed, char[] fileChars) {
    ...
    String Character = String.valueOf(fileChars[0]) + "," + String.valueOf(fileChars[1]);
    for (int i=0; i < compressed.length; i++ ){
      compressedFile = compressedFile + "\n" + compressed[i];
    }
    compressedFile = Character + compressedFile + "\n";
   ...
   */
   public String decompressLine(String line, char[] fileChars){
      ...
      String char0 = String.valueOf(fileChars[0]);
      String char1 = String.valueOf(fileChars[1]);

      for (int i=0; i < newLine.length; i++){
        if (i%2 ==0){
          for (int k=0; k < Integer.parseInt(newLine[i]); k++){
            decompressedLine = decompressedLine + char0;
          }
        }
        else {
          for (int j=0; j < Integer.parseInt(newLine[i]); j++){
            decompressedLine = decompressedLine + char1;
          }
        }
      }
  ...
  public String[] decompressAllLines(String[] lines, int dataSize){
     //TODO: Implement this method
     String[] decompressedAllLine = new String[dataSize];
     char[] fileChars = new char[2];
     String[] firstLine = lines[0].split(",");
     fileChars[0] = firstLine[0].charAt(0);
     fileChars[1] = firstLine[1].charAt(0);
     for (int i=1; i < dataSize; i++){
       decompressedAllLine[i] = decompressLine(lines[i], fileChars);
     }
      return decompressedAllLine;
  }
