...
public class ArrayHashTable<K, V> implements HashTable<K, V> {
  ...
  private void resizeArray() {
   //TODO: Implement this method
    ...
    for (int i = 0; i < keyArray.length; i++){
      if (keyArray[i] != null && isActiveArray[i]){
        int hash = hash(keyArray[i]);
        hash = collisionHandler.probe(hash, newActive, capacity);
        newKeyArray[hash] = keyArray[i];
        newValueArray[hash] = valueArray[i];
        newActive[hash] = true;
      }
    }
  ...
  */
  private double calcLoadFactor() {
   //TODO: Implement this method
    double factor = ((double)(count))/capacity; 
      return factor;
  } 
  ...
  public void put(K key, V value) {
      ...
      if (key != null){
        int index = hash(key);
        int hashedKey = getIndex(index, key);
        if(hashedKey > 0){
          valueArray[hashedKey] = value;
        }
        else {
  ...
  //  while (keyArray[index] != null && bucketProbed < capacity){
  //    if (keyArray[index].equals(target)){
  ...
  public V remove(K targetKey)throws ElementNotFoundException {
   //TODO: Implement this method
   if (targetKey == null){
     throw new ElementNotFoundException("Target does not exist in the table");
   }
   ...
   while (keyArray[index] != null && bucketProbed < capacity){
     if (keyArray[index].equals(targetKey)){
